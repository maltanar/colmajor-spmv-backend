...
for peCount in $PE_COUNTS; do
  for i in {0..9}; do
    ...
    colcnt=${col_counts[$i]}
    echo "./addrgen-baseline $nzcnt $colcnt $peCount > $OUTPUT_DIR/k6_${matrix}_${peCount}_baseline.trc"
    ./addrgen-baseline $nzcnt $colcnt $peCount > $OUTPUT_DIR/k6_${matrix}_${peCount}_baseline.trc
  done
done
